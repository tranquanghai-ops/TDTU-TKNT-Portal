 ...
 *     a. Locate the app artifact zip (from build-artifacts/)
...
import { readFileSync, mkdirSync, rmSync, existsSync, cpSync, readdirSync } from 'fs';
...
for (const app of enabledApps) {
  const mountSegment = app.mount.replace(/^\/|\/$/g, '');
  ...
  log(`  ok Extracted ${extracted.length} file(s) to build/${mountSegment}/`);
}
